...
ssconvert ‘Tableau des médailles v2.xlsx’ ‘Tableau des médailles.csv’
...
tail -n +6 "$CLEANED_TEMP_FILE" | cut -d',' -f4- > "$CLEANED_FINAL_FILE"
...
awk -F',' 'BEGIN { OFS=","; rank=1; prev_gold=-1; prev_silver=-1; prev_bronze=-1 }
    NR==1 { print "Rank," $0; next } # Ajouter un en-tête
    {
        # Si les médailles sont identiques au précédent, on affiche "-"
